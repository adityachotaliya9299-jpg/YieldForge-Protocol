    ...
    try {
      ...
      if ((prefs.emailEnabled && prefs.email) || (prefs.telegramEnabled && prefs.telegramChatId)) {
        await fetch("/api/alerts/register", {
          ...
          body: JSON.stringify({
            type:       prefs.emailEnabled ? "email" : "telegram",
            address,
            contact:    prefs.emailEnabled ? prefs.email : prefs.telegramChatId,
            lockEnd:    lockEndTimestamp,
            daysBefore: prefs.daysBeforeAlert,
          }),
        });
      }
  ...
  return (
    <div className="yf-card" style={{ overflow:"hidden" }}>
      ...
      <button onClick={() => setOpen(o => !o)}
        style={{ width:"100%", padding:"1.1rem 1.5rem", background:"none", border:"none", cursor:"pointer", display:"flex", justifyContent:"space-between", alignItems:"center", fontFamily:"inherit" }}>
      ...
      {open && (
        <div style={{ padding:"0 1.5rem 1.5rem", borderTop:"1px solid var(--border)" }}>
          ...
          <div style={{ marginBottom:"0.75rem", padding:"1rem", background:"var(--bg-input)", border:"1px solid var(--border)", borderRadius:"12px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: prefs.emailEnabled ? "0.75rem" : 0 }}>
            ...
            {prefs.emailEnabled && (
              <input type="email" placeholder="your@email.com"
                value={prefs.email} onChange={e => setPrefs(p => ({...p, email:e.target.value}))}
          ...
          <div style={{ marginBottom:"1rem", padding:"1rem", background:"var(--bg-input)", border:"1px solid var(--border)", borderRadius:"12px" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom: prefs.telegramEnabled ? "0.75rem" : 0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:"0.55rem" }}>
            ...
            {prefs.telegramEnabled && (
              <div style={{ display:"flex", flexDirection:"column", gap:"0.5rem" }}>
                <a href="https://t.me/userinfobot" target="_blank" rel="noopener noreferrer"
                  style={{ display:"block", padding:"0.65rem", background:"rgba(0,136,204,0.1)", border:"1px solid rgba(0,136,204,0.3)", borderRadius:"8px", color:"#0088cc", textDecoration:"none", fontSize:"0.83rem", fontWeight:600, textAlign:"center" }}>
                  ✈️ Get your Chat ID from @userinfobot →
                </a>
                <input type="text" placeholder="Paste your Chat ID here (e.g. 123456789)"
